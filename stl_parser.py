...
class STLVisitor(NodeVisitor):
    ...
    def visit_global(self, _, (_1, interval, phi)): return stl.G(phi, interval)
...
class STL2Visitor(NodeVisitor):
    def generic_visit(self, _, children): return children
    def visit_phi(self, _, children): return children[0]
    def visit_env(self, _, (phi, _1)): return phi
    def visit_sys(self, _, (phi, _1, _2)): 
...
def parse_stl2(stl_str):
    return STL2Visitor().visit(STL2_GRAMMAR.parse(stl_str))

def main():
    print(parse_stl("⋄[1,2](x1 < 2)"))
    print(parse_stl("¬(⋄[1,2](x1 < 2))"))
    print(parse_stl("(□[1,3](x1 < 2)) ∨ (x2 >= 4)"))
    print(parse_stl("(⋄[1,3](□[1,3](x1 < 2))) ∧ (x2 >= 4)"))
    with open('example1.stl', 'r') as f:
        print(parse_stl2("".join(f.readlines())))
